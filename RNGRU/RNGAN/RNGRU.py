...
if __name__ == '__main__':
    ...
    data = np.array(pd.read_csv(r'D:\Pycharmwenjian\RNGRU\train_分层.csv', error_bad_lines=False, lineterminator="\n", encoding="gbk"))
    datax = data[:,:-1]
    datay = data[:,-1]
    ...
    for epoch in range(nepoch):
        for i, (data,label) in enumerate(dataloader, 0):   
            ...
            output = generator.forward(torch.from_numpy(huigui).to(torch.float32),torch.from_numpy(jinlin).to(torch.float32),z)  
